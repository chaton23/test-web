...
#- リソースグループ作成
RGNAME=user0004-webapp-image${BUILD_NUMBER}-rg
...
az vm create -n websv0 \
    --public-ip-address webapp-pip --nsg webapp-websv-nsg \
    --size Standard_F1 --image OpenLogic:CentOS:7.3:latest \
    --storage-sku Standard_LRS \
    --admin-username devops --ssh-key-value ~/.ssh/id_rsa.pub
